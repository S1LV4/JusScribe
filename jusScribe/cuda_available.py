...
class CudaInfo:
    ...
    def from_json_str(json_str: str) -> "CudaInfo":
        ...
        assert json_str is not None, "Esperava-se que json_str fosse definido, mas em vez disso foi Nenhum"
...
def cuda_cards_available() -> CudaInfo:
    ...
    if torch.cuda.is_available():
        ...
        try:
            ...
            sys.stderr.write(f"Erro ao obter a contagem de dispositivos: {e}\n")
            return CudaInfo(False, 0, [])
        for i in range(count):
            try:
                ...
                sys.stderr.write(f"Erro ao obter o dispositivo {i}: {e}\n")
...
def parse_args() -> argparse.Namespace:
    """Parse the arguments."""
    parser = argparse.ArgumentParser(description="Verificar se CUDA está disponível.")
...
def main() -> int:
    ...
    assert json_str is not None, "Esperado json_str a ser definido, mas foi None"
    # print(json_str)
